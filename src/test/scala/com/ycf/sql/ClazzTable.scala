...
import com.ycf.flink.graph.TablePoint
...
/**
  ...
  */
class ClazzTable extends TablePoint{
  /**
    * table API 操作
    *
  ...
  override def process(tenv: StreamTableEnvironment, dataStream: DataStream[MsgContext]): DataStream[MsgContext] = {
    val ds = dataStream.map(x => {
      ...
      (json.get("cid").toString.toLong, json.get("cname").toString)
    })

    tenv.createTemporaryView("clazz", ds, 'cid, 'cname)
    null
  }
}
